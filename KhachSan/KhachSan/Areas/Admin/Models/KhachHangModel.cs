...
namespace KhachSan.Areas.Admin.Models
{
    public class KhachHangModel
    {
        ...
        {
            c = new ConnectDb();
            List<DANH_MUC_LOAI_GIAY_TO> list = new List<DANH_MUC_LOAI_GIAY_TO>();
            SqlCommand cmd = new SqlCommand("getAllLoaiGiayTo", c.conn);
            cmd.CommandType = CommandType.StoredProcedure;
            rdr = cmd.ExecuteReader();
            while (rdr.Read())
            {
                DANH_MUC_LOAI_GIAY_TO item = new DANH_MUC_LOAI_GIAY_TO();
                item.MALOAIGIAYTO = (int)rdr["MALOAIGIAYTO"];
                item.TENLOAIGIAYTO = (string)rdr["TENLOAIGIAYTO"];              
                list.Add(item);
            }
            return list;
        }
